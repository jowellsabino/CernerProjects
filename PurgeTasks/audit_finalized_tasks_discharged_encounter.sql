...
WHERE ta.task_status_cd+0 IN (SELECT code_value 
                               ...
                               WHERE code_set = 79
                                 ...
                                 AND code_value IN (SELECT code_value              
                                                     ...
                                                     WHERE field_value IN ('1','2','4'))) /* 1- Cancelled, Complete, Deleted, Discontinued (task statuses)*/
  ...
  AND ta.updt_dt_tm < now() - 90 /* Should set to 90 since 90 days is max in purge rules.  If count(*) > 0, these are unpurged */
...
ORDER BY TASK_TYPE, PURGETASKSTATUS, TASk_STATUS 
;


/* Get rows for non-phone message tasks */
SELECT cvtt.display AS TASK_TYPE
     , cvts.display AS TASK_STATUS
     , ta.ACTIVE_IND /* Deleting a task puts deleted as task_status in task_activity_assignment, not task-activity */
--     , CASE 
--            WHEN  tpc.task_status_flag  = 0 THEN 'Finalized'
--            WHEN  tpc.task_status_flag  = 1 THEN 'Dropped'
--            WHEN  tpc.task_status_flag  = 2 THEN 'Active'
--            ELSE  'Unknown'
--       END AS PurgeTaskStatus
--     , tpc.retention_days
--     , tpc.purge_active_flag
...
WHERE ta.task_status_cd+0 IN (SELECT code_value 
  ...
  AND ta.task_type_cd+0 IN (SELECT code_value
                             ...
                             WHERE  code_set = 6026
                               AND cdf_meaning NOT IN ('ORDER', 'ENDORSE', 'PHONE MSG','NEW RESULT')) 
   AND ta.updt_dt_tm < now() - 90 /* Should set to 90 since 90 days is max of display filter.  If count(*) > 0, these are unpurged */
GROUP BY cvtt.display
     , cvts.display
     , ta.ACTIVE_IND 
--     , tpc.task_status_flag
...
/* Detailed query of above -- this may return a lot of rows */
select ta.task_id
     , e.encntr_id
     , ta.updt_dt_tm
     , cvtt.display AS TASK_TYPE
     , cvts.display AS TASK_STATUS
     , ta.ACTIVE_IND 
     , ta.*
from task_activity ta
...
where ta.task_status_cd+0 in (select code_value 
                               ...
                               where code_set = 79
                                 ...
                                 and code_value in (select code_value              
                                                      from code_value_extension
                                                     where field_value = '1')) /* 1- Cancelled, Complete, Deleted, Discontinued - corresponding task statuses */
                                                     /* 2 - Dropped; 4 - Delivered, In Error, On Hold, Opened
                                                                       , Overdue, Pending, Read, Rework, Suspended
                                                                       , Refused, Pending Validation, Recalled */
  and ta.task_type_cd+0 in (select code_value
                              from code_value
                             where code_set = 6026
                               and cdf_meaning not in ('ORDER', 'ENDORSE', 'NEW RESULT')) /* 'PHONE MSG' */
  and ta.updt_dt_tm < now() - 90 /* Should set to 90 since 90 days is max in purge rules */
order by TASK_TYPE
;

/* Query FOR purge rules that may not work as a result of the defect
 * A generic purge rule sis where task_type_cd is null
 */
